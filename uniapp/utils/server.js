import { WSS_SERVER_URL, HEADER, SERVER_DEBUG, PINGINTERVAL} from '@/config/socket.js';
  ...
  */
  $on(name, successFun) {
    ...
    if (typeof name === 'object') {
      name.forEach(item => {
        if (!that.watcherList[item]) this.watcherList[name] = [];
        that.watcherList[item].push(successFun);
      })
    } else {
