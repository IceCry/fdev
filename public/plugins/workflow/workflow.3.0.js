    ...
    var setConnections = function (conn, remove) {
        ...
        if (aConnections.length > 0) {
            ...
            for (var j = 0; j < aConnections.length; j++) {
                var from = $('#' + aConnections[j].sourceId).attr('step_id');
                var target = $('#' + aConnections[j].targetId).attr('step_id');
    ...
    $.fn.Flowdesign = function (options) {
        ...
        if (processData.list) {
            $.each(processData.list, function (i, row) {
                ...
                if(row.step_name!='开始' && row.step_name!='结束'){
                    var btnStr = '<span class="process-flag fa fa-link line1" style="text-align: left;width: 180px;">&nbsp;&nbsp;' + row.step_name + '</span>';
                }else{
                    var btnStr = '<span class="line1" style="text-align: left;width: 180px;">&nbsp;&nbsp;' + row.step_name + '</span>';
                }
                if(row.checker_type){
                    btnStr += '<br/><span style="width: 180px;" class="fa fa-dot-circle-o line1">&nbsp;&nbsp;' + row.checker_type + '</span>';
                }
                if(row.checker_name){
                    btnStr += '<br/><span style="width: 180px;" class="fa fa-user line1">&nbsp;&nbsp;' + row.checker_name + '</span>';
                }

                $(nodeDiv).attr("id", nodeId)
                    .attr("style", row.style + 'text-align: left;')
                    .attr("next_step_id", row.next_step_id)
                    .attr("step_id", row.id)
                    .attr("condition", row.condition_str)
        ...
        $(".process-step").live('click', function () {
            _canvas.find('#wf_active_id').val($(this).attr("step_id")),
        ...
        function mtAfterDrop(params) {
            defaults.mtAfterDrop({
                sourceId: $("#" + params.sourceId).attr('step_id'),
                targetId: $("#" + params.targetId).attr('step_id')
            });
        }
        ...
        jsPlumb.makeTarget(jsPlumb.getSelector(".process-step"), {
            ...
            beforeDrop: function (params) {
                ...
                $('#wf_process_info').find('input').each(function (i) {
                    var str = $('#' + params.sourceId).attr('step_id') + ',' + $('#' + params.targetId).attr('step_id');
                    if (str == $(this).val()) {
                        j++;
                        return;
                    }
        ...
        var _canvas_design = function () {
            $('.process-step').each(function (i) {
                var sourceId = $(this).attr('step_id');
                var prcsto = $(this).attr('next_step_id');
                    ...
                    $("#wf_process_info input[type=hidden]").each(function (i) {
                        ...
                        if (processVal.length == 2) {
                            if (!aProcessData[processVal[0]]) {
                                aProcessData[processVal[0]] = {"top": 0, "left": 0, "next_step_id": []};
                            }
                            aProcessData[processVal[0]]["next_step_id"].push(processVal[1]);
                        }
                    })
                    _canvas.find("div.process-step").each(function (i) { //生成Json字符串，发送到服务器解析
                        if ($(this).attr('id')) {
                            var pId = $(this).attr('step_id');
                            var pLeft = parseInt($(this).css('left'));
                            var pTop = parseInt($(this).css('top'));
                            if (!aProcessData[pId]) {
                                aProcessData[pId] = {"top": 0, "left": 0, "next_step_id": []};
                            }
