...
/**
 * Class YiLianYun
 * @package sensen\services\printer\storage
 */
class YiLianYun extends BasePrinter
{
    ...
    public function setPrinterContent(array $config): self
    {
        ...
        foreach ($product as $item) {
            $goodsStr .= '<tr>';
            $truePrice = bcmul((string)$item['truePrice'], (string)$item['cart_num'], 2);
            $goodsStr .= "<td>{$item['productInfo']['store_name']}</td><td>{$item['cart_num']}</td><td>{$item['productInfo']['price']}</td><td>{$truePrice}</td>";
            $goodsStr .= '</tr>';
        }
        $goodsStr .= '</table>';
